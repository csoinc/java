...
import com.oyou.gwt.album.client.AlbumDataProvider.AlbumDataAcceptor;

/**
 * 
 * @author	Owen Ou
 * @version $Id: AlbumList.java,v 1.1 2008/06/29 14:24:25 oyou Exp $
 * @since Nov 14, 2007
 */
public class AlbumList extends Composite implements TableListener, ChangeListener, ClickListener {
	
	private class AlbumDataAcceptorImpl implements AlbumDataAcceptor {
		public void accept(IsPhoto[] datas) {
			setPhotos(datas);
		}

		public void acceptAlbums(IsAlbum[] datas) {
			setAlbums(datas);
		}

		public void acceptCategories(IsCategory[] datas) {
			setCategories(datas);
		}
		...
		public void failed(Throwable caught) {
			...
			} if (caught instanceof AlbumException) {
	...
	private HTML hSelection = new HTML("OYOAlbum v1.0<br>Release 11/30/2007<br>相册Album选择Selection", true);
	private HTML hLoading = new HTML("OYOAlbum v1.0<br>Release 11/30/2007<br><font color=ff0000><blink>Loading......</blink></font>", true);
	...
	private HTML slideButton = new HTML("<a href='javascript:;'>slideshow</a>", true);
	...
	private AlbumDataAcceptor acceptor = new AlbumDataAcceptorImpl();

	private AlbumDataProvider provider = null;

	private ErrorDialog errorDialog = null;

	private IsPhoto[] photos = null;

	private IsAlbum[] albums = null;

	private IsCategory[] categories = null;
	...
	public AlbumList(AlbumDataProvider provider) {
	...
	public void onClick(Widget sender) {
		if (sender == olderButton) {
			...
			if ("slideshow".equals(slideButton.getText())) {
				int index = startIndex + VISIBLE_PHOTO_COL * selectedRow + selectedCell;
				if (index < 0) index = 0;
				Album.get().slidePhotos(photos, index + 1);
			} else {
				Album.get().stopSlide();
			}
		}
	...
	private void selectRow(int row, int cell) {
		...
		if (index <= photos.length) {
			...
			if (photo != null) {
				Album.get().displayPhoto(photo);
			}
		}
	}

	private void setPhotos(IsPhoto[] photos) {
		this.photos = photos;
		update();
	}

	private void setAlbums(IsAlbum[] albums) {
		this.albums = albums;
		lbAlbum.clear();
		for (int i = 0; i < albums.length; i++) {
			IsAlbum album = albums[i];
	...
	private void setCategories(IsCategory[] categories) {
		this.categories = categories;
		lbCategory.clear();
		for (int i = 0; i < categories.length; i++) {
			IsCategory category = categories[i];
		...
		for (int i = 0; i < VISIBLE_PHOTO_ROW; ++i) {
			for (int j = 0; j < VISIBLE_PHOTO_COL; ++j) {
				if (startIndex + i * VISIBLE_PHOTO_COL + j >= photos.length) {
					...
					IsPhoto photo = photos[startIndex + i * VISIBLE_PHOTO_COL + j];
	...
	public void onChange(Widget sender) {
		if (sender == lbCategory) {
			...
			IsCategory category = categories[cIndex];
