 ...
 * 
 * @author	Owen Ou
 * @version $Id: MyAlbumList.java,v 1.1 2008/06/29 14:23:48 oyou Exp $
 * @since Dec 5, 2007
 */
public class MyAlbumList extends Composite implements TableListener, ChangeListener, ClickListener {
	...
	private HTML hSelection = new HTML("Blogger2<br> 2010<br>相册Album选择Selection", true);
	private HTML hLoading = new HTML("Blogger2<br> 2010<br><font color=ff0000><blink>Loading......</blink></font>", true);
	...
	public MyAlbumList(BlogDataProvider provider) {
	...
	public void onClick(Widget sender) {
		if (sender == olderButton) {
			...
			if ("slide".equals(slideButton.getText())) {
				...
				if (index < 0) index = 0;
				Blog.get().slideMyPhotos(photos, index + 1);
			} else {
				Blog.get().stopMySlide();
			}
		}
	...
	private void setAlbums(IsGroup[] albums) {
		...
		provider.updatePhotos("", albums[index].getId().toString(), Blog.UID, acceptor);
	}
	...
	public void onChange(Widget sender) {
		if (sender == lbCategory) {
			...
			if (aIndex >= albums.length) {
				provider.updatePhotos(category.getId().toString(), "", Blog.UID, acceptor);
			} else {
				IsGroup album = albums[aIndex];
				provider.updatePhotos(category.getId().toString(), album.getId().toString(), Blog.UID, acceptor);
			}
		}
	}	
