...
public class MyArticleList extends Composite implements TableListener, ChangeListener, ClickListener {
	...
	private HTML hSelection = new HTML("Blogger2<br> 2010<br>文章Article选择Selection", true);
	...
	public MyArticleList(BlogDataProvider provider) {
	...
	private void selectRow(int row, int cell) {
		...
		if (index <= articles.length) {
			...
			if (article != null) {
				Blog.get().displayMyArticle(article);
			}
		}
	}
	...
	private void setGroups(IsGroup[] groups) {
		...
		provider.updateArticles("", groups[index].getId().toString(), Blog.UID, acceptor);
	}
	...
	public void onChange(Widget sender) {
		if (sender == lbCategory) {
			...
			if (aIndex >= groups.length) {
				provider.updateArticles(category.getId().toString(), "", Blog.UID, acceptor);
			} else {
				IsGroup group = groups[aIndex];
				provider.updateArticles(category.getId().toString(), group.getId().toString(), Blog.UID, acceptor);
			}
		}
	}	
